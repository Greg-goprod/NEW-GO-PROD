-- Migration: Extension du RPC pour inclure TOUTES les métriques Songstats
-- Cette fonction remplace la version précédente en ajoutant stats_all et stats_list
...
-- FULL METRICS (per source -> per metric)
stats_grouped AS (
  SELECT
    s.source,
    jsonb_object_agg(
...
stats_all AS (
  SELECT COALESCE(jsonb_object_agg(source, metrics ORDER BY source), '{}'::jsonb) AS j
  FROM stats_grouped
),
stats_list AS (
  SELECT jsonb_agg(
...
info AS (
  SELECT jsonb_build_object(
    ...
    'last_updated_any', GREATEST(
      (SELECT last_stats_updated_at FROM stats_high),
...
SELECT jsonb_build_object(
  'stats',      COALESCE((SELECT to_jsonb(stats_high.*) FROM stats_high), '{}'::jsonb),
  'stats_all',  COALESCE((SELECT j FROM stats_all), '{}'::jsonb),
  'stats_list', COALESCE((SELECT j FROM stats_list), '[]'::jsonb),
